...
from pydantic import BaseModel # Field was imported but not used, removing
import os
import logging
import json
...
class SelectedSegmentMarker(BaseModel):
    start: float # Gemini will return the start time of the segment it wants to keep
...
def generate_narrative_outline(transcript_text: str, user_prompt: str) -> list[str]:
    ...
    try:
        ...
        if not GEMINI_API_KEY_MODULE_LEVEL:
            logger.error("GEMINI_API_KEY not available for Pass 1.")
            raise RuntimeError("GEMINI_API_KEY environment variable not set.")
        
        local_genai.configure(api_key=GEMINI_API_KEY_MODULE_LEVEL)
        logger.info("Pass 1: Gemini API key configured.")

    except ImportError:
        logger.error("Pass 1: Failed to import google.generativeai.")
        raise RuntimeError("Critical: google.generativeai library not found for Pass 1.")
    except Exception as e_conf:
        logger.error(f"Pass 1: Failed to configure Gemini API key: {e_conf}")
        raise RuntimeError(f"Pass 1: Failed to configure Gemini API key: {e_conf}")
...
Output ONLY the JSON list of strings and nothing else."""
    ...
    logger.info(f"Pass 1: Sending request to Gemini for narrative outline. User prompt: '{user_prompt}'. Transcript length: {len(transcript_text)} chars.")
    
    try:
        model_identifier = 'models/gemini-2.5-pro-exp-03-25' # Using existing model from current code
        logger.info(f"Pass 1: Using Gemini model: {model_identifier}")
        model = local_genai.GenerativeModel(model_identifier)
        
        generation_config_dict = {
            "response_mime_type": "application/json",
            # No explicit Pydantic response_schema here as we expect a simple list[str]
            # The prompt clearly asks for JSON list of strings.
        }
        ...
        if not response.text:
            logger.warning("Pass 1 Gemini returned empty response text. Returning empty narrative outline.")
            return []
        ...
        try:
            ...
            if not (isinstance(narrative_points_data, list) and \
               all(isinstance(item, str) for item in narrative_points_data)):
                logger.error(f"Pass 1 Gemini response not a list of strings as expected. Data: {str(narrative_points_data)[:500]}")
                logger.error(f"Full problematic Pass 1 Gemini response text: '{response.text}'")
                return [] # Or raise error
            logger.info(f"Pass 1 Gemini returned {len(narrative_points_data)} narrative points.")
            return narrative_points_data
        except json.JSONDecodeError as json_e:
            logger.error(f"Pass 1 Failed to decode JSON from Gemini: {json_e}.")
            logger.error(f"Problematic Pass 1 Gemini response text: '{response.text[:5000]}'")
            raise RuntimeError("Pass 1 Gemini returned malformed JSON for narrative outline.")

    except AttributeError as ae_model: 
        logger.error(f"Pass 1 AttributeError when trying to use local_genai.GenerativeModel or related: {ae_model}", exc_info=True)
        raise RuntimeError(f"Pass 1 Gemini model instantiation or method call failed: {ae_model}")
    except Exception as e:
        logger.error(f"Pass 1 Gemini API call failed: {e}", exc_info=True)
        raise RuntimeError(f"Pass 1 Gemini content generation failed: {e}")
...
-   **Exclusions (Strict):**
    -   Do NOT select segments where the speaker explicitly reads out or directly addresses individual user/chatter names or messages. General acknowledgments of "chat" are okay if part of a broader funny moment.
    -   Avoid purely transitional or filler content unless it's exceptionally brief AND inherently funny/characteristic AND directly serves a Narrative Outline point.
...
If no segments from the "Current Transcript Chunk" meet the criteria based on the Narrative Outline and User Prompt, return an empty JSON list: `[]`.
    ...
    try:
        ...
        for i, marker_data in enumerate(selected_markers_data):
            if not isinstance(marker_data, dict) or 'start' not in marker_data:
                logger.warning(f"Pass 2 Item {i} in Gemini response is not a valid marker (dict with 'start'). Skipping: '{str(marker_data)[:100]}'")
                continue
            try:
